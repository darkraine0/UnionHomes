...
def get_plans(db: Session = Depends(get_db)):
    ...
    result = []
    for plan in plans:
        # Only include plans with all required fields present
        if not (plan.plan_name is not None and plan.price is not None and plan.sqft is not None and plan.stories is not None and plan.price_per_sqft is not None and plan.company and plan.community):
            continue
