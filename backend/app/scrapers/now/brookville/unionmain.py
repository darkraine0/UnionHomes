...
class UnionMainBrookvilleNowScraper(BaseScraper):
    URL = "https://unionmainhomes.com/all-homes/?nh=brookville-estates"
    ...
    def fetch_plans(self) -> List[Dict]:
        try:
            ...
            print(f"[UnionMainBrookvilleNowScraper] Response status: {resp.status_code}")
            
            if resp.status_code != 200:
                print(f"[UnionMainBrookvilleNowScraper] Request failed with status {resp.status_code}")
                return []
            ...
            print(f"[UnionMainBrookvilleNowScraper] Found {len(home_listings)} home listings")
            
            for idx, listing in enumerate(home_listings):
                try:
                    print(f"[UnionMainBrookvilleNowScraper] Processing listing {idx+1}")
                    ...
                    if not title_link:
                        print(f"[UnionMainBrookvilleNowScraper] Skipping listing {idx+1}: No title link found")
                        continue
                    
                    address = title_link.get_text(strip=True)
                    if not address:
                        print(f"[UnionMainBrookvilleNowScraper] Skipping listing {idx+1}: Empty address")
                        continue
                    
                    # Check for duplicate addresses
                    if address in seen_addresses:
                        print(f"[UnionMainBrookvilleNowScraper] Skipping listing {idx+1}: Duplicate address '{address}'")
                        continue
                    ...
                    if not price_li:
                        print(f"[UnionMainBrookvilleNowScraper] Skipping listing {idx+1}: No price found")
                        continue
                    
                    current_price = self.parse_price(price_li.get_text())
                    if not current_price:
                        print(f"[UnionMainBrookvilleNowScraper] Skipping listing {idx+1}: No current price found")
                        continue
                    ...
                    if not sqft:
                        print(f"[UnionMainBrookvilleNowScraper] Skipping listing {idx+1}: No square footage found")
                        continue
                    ...
                    print(f"[UnionMainBrookvilleNowScraper] Listing {idx+1}: {plan_data}")
                    listings.append(plan_data)
                    
                except Exception as e:
                    print(f"[UnionMainBrookvilleNowScraper] Error processing listing {idx+1}: {e}")
                    continue
            
            print(f"[UnionMainBrookvilleNowScraper] Successfully processed {len(listings)} listings")
            return listings
