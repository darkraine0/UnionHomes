...
class UnionMainElevonScraper(BaseScraper):
    ...
    def fetch_plans(self) -> List[Dict]:
        try:
            print(f"[UnionMainElevonScraper] Fetching URL: {self.URL}")
            headers = {
                "User-Agent": (
            ...
            print(f"[UnionMainElevonScraper] Response status: {resp.status_code}")
            soup = BeautifulSoup(resp.text, "html.parser")
            listings = []
            cards = soup.select('section#fp .item-listing-wrap')
            print(f"[UnionMainElevonScraper] Found {len(cards)} home cards.")
            for idx, card in enumerate(cards):
                price_tag = card.select_one('.item-price')
                price = self.parse_price(price_tag.get_text() if price_tag else "")
                if price is None:
                    print(f"[UnionMainElevonScraper] Skipping card {idx+1}: Missing price.")
                    continue
                ...
                print(f"[UnionMainElevonScraper] Card {idx+1}: {plan_data}")
                listings.append(plan_data)
            return listings
        except Exception as e:
            print(f"[UnionMainElevonScraper] Error: {e}")
            return [] 